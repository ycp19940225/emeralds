...
class ChatController extends BaseController
{
    ...
    public function newest(Request $request){
        ...
        $request->input('lasttime')?$time=$request->input('lasttime'):$time=time();
        $User = DB::table('emerald_chat'); // 实例化User对象
        $list = $User->whereRaw('(uid='.$shopid.' and touid='.$touid.' and created_at <'.$time.')or(uid='.$touid.' and touid='.$shopid.' and created_at <'.$time.')')->where('state',1)->orderBy('created_at','desc')->get();
        $sa['state']=2;
